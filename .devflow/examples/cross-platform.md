...
> PowerShell ↔ Bash equivalents for DevFlow workflows
...
**Bash:**
```bash
if grep -q "FINALIZED" .devflow/SPEC.md; then echo "Ready"; fi
```
...
**PowerShell:**
```powershell
Get-ChildItem ".devflow/phases/1/*-PLAN.md"
```

**Bash:**
```bash
ls .devflow/phases/1/*-PLAN.md 2>/dev/null
```
...
```markdown
**Note:** Commands shown are PowerShell. For Bash equivalents, see `.devflow/examples/cross-platform.md`
```
